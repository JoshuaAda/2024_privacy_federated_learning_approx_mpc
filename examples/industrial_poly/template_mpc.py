...
def template_mpc(model, silence_solver = False):
    ...
    mterm = - model.x['m_P']+model.tvp['cost_value']*model.x['Q']
    lterm = - model.x['m_P']+model.tvp['cost_value']*model.x['Q']#+0.1*model.u['T_in_M']+0.1*model.u['T_in_EK']
    ...
    mpc.bounds['lower','_x','T_R'] = T_set - temp_range
    ...
    mpc.bounds['upper','_x','T_R'] = T_set + temp_range
    ...
    if mpc.settings.n_robust == 0:
       # Sot-constraint for the reactor upper bound temperature
        mpc.set_nl_cons('T_R_UB', model.x['T_R'], ub=T_set+temp_range, soft_constraint=True, penalty_term_cons=1e4)
    else:
        mpc.bounds['upper','_x','T_R'] = T_set+temp_range
    ...
    #mpc.set_uncertainty_values(cost_value=np.array([10]))
    template=mpc.get_tvp_template()
    def tvp_fun(t_curr):
        for k in range(mpc.settings.n_horizon+1):
            template['_tvp',k,'cost_value'] = 10
        return template
